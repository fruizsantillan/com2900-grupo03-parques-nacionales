...
BEGIN
    ...
    BEGIN TRY
        ...
        USING (
            SELECT
                DATEFROMPARTS(
                    TRY_CAST(PARSENAME(REPLACE(LTRIM(RTRIM(indiceTiempo)), '-', '.'), 3) AS INT),
                    TRY_CAST(PARSENAME(REPLACE(LTRIM(RTRIM(indiceTiempo)), '-', '.'), 2) AS INT),
                    1
                )                                                  AS periodo,
                LOWER(LTRIM(RTRIM(origenVisitantes)))              AS origenVisitante,
                TRY_CAST(NULLIF(LTRIM(RTRIM(visitas)), '') AS INT) AS cantidadVisitas,
                NULLIF(LTRIM(RTRIM(observaciones)), '')            AS observaciones
            FROM #VisitasNacionales
            WHERE TRY_CAST(NULLIF(LTRIM(RTRIM(visitas)), '') AS INT) IS NOT NULL -- Ignora filas con letras en 'visitas'
              AND TRY_CAST(PARSENAME(REPLACE(LTRIM(RTRIM(indiceTiempo)), '-', '.'), 3) AS INT) IS NOT NULL -- Ignora fechas rotas
        ) AS origen
